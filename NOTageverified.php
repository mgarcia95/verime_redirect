...
if (!isset($_GET["av"])) {
	...
	if ($av !== "false") {
		if (!isset($_GET["av"])) {
			// Log Error
			error_log(__FILE__.": av parameter not [false]");
			$security_error = true;	
		}
	}
}
...
if (!isset($_GET["tid"])) {
    ...
    // Transaction ID should be store for your records
    // If Transaction ID = 0: User cancelled AV or AV error, so no AV attempted
    $tid = $_GET["tid"];
}    
...
<html>
<head>
<title>VeriMe Redirect API DEMO - NOT Age-Verified</title>
<style type="text/css">
<!--
body {
...
<body>
    <span style="color:red;font-weight:bold">Sorry. We cannot establish Age-Verification.</span>
</body>
</html>